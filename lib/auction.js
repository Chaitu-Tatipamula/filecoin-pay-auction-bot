...
import { SUSHISWAP_NATIVE_PLACEHOLDER, executeSwap, getSwapQuote } from './swap.js'
...
async function estimateGasCosts({
  ...
  const [swapGasEstimate, gasPrice] = await Promise.all([
    swapEnabled && swapTx
 ...
 * @returns {{ totalCost: bigint; isProfitable: boolean; }}
 */
...
export async function processAuctions({
  ...
  // Check profitability and balance
  const swapAmountOut = BigInt(swapQuote.assumedAmountOut)
  const { totalCost, isProfitable } = checkProfitability({
    auctionPrice,
    bidGasCost: gasCosts.bidGasCost,
    swapGasCost: gasCosts.swapGasCost,
    swapAmountOut
  })
